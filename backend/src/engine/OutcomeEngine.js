/**
 * OutcomeEngine.js — Module D
 * All point calculations and bet validation. Pure functions — no side effects.
 */

function calculateOutcome(entryPrice, exitPrice, stake, multiplier = 1, direction = 'up') {
...
function resolveTimedBet(bet, currentPrice) {
...
function checkSmartAlert(entryPrice, currentPrice, threshold = 2) {
  const pct = ((currentPrice - entryPrice) / entryPrice) * 100;
  return Math.abs(pct) >= threshold ? pct : null;
}

function rollDoubleOrNothing() {
  return Math.random() > 0.5 ? 2 : 0;
}

module.exports = { calculateOutcome, resolveTimedBet, getMaxStake, validateBet, checkSmartAlert, rollDoubleOrNothing };
